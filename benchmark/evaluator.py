...
def eval_multiple_choice(model, tokenizer, context, choices, label_idx, max_length=512):
    ...
    for choice in choices:
        # 每个选项都和上下文拼接变成一个完整的输入
        full_text = context + choice
        
        # 使用 tokenizer 编码输入文本，得到 token_id 序列
        # （添加最大长度限制：训练的时候用多长的 seq_len ，评测的时候也用一样的）
        inputs = tokenizer(
            full_text, 
            return_tensors='pt', 
            truncation=True, 
        ...
        # 计算 context 的长度（用于定位 choice 的起始位置，只有 choice 才算 loss）
        ...
        # 在 loss 中，预测位置 context_len 的 token 对应 loss[context_len-1]，即 context 的最后一个 token 预测 choice 的第一个 token
        ...
        # 如果 context 太长，导致 choice 被截掉，那么就使用整个输入的 loss
        else:
...
def eval_c3(model, tokenizer, data_path):
    ...
    with open(data_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # 评测
            correct += eval_multiple_choice(model, tokenizer, full_context, choices, label_idx)
            total += 1
...
def eval_xcopa(model, tokenizer, data_path):
    ...
    with open(data_path, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # 评测
            correct += eval_multiple_choice(model, tokenizer, context, choices, label_idx)
            total += 1

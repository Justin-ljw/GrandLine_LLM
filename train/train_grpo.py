...
def calculate_rewards(raw_prompts, responses, args):
    ...
    # 1. 格式检查（只有严格匹配 CoT 格式的回答才能参与后续的内容评分，格式不匹配的直接判 0 分）
...
def compute_logprobs(model, outputs, completion_len):
    """
    计算 completion 部分每个 token 的 log probability ，用于计算 KL散度
...
def train_epoch(epoch, loader, iters, model, ref_model, optimizer, tokenizer, autocast_ctx, args,
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # 核心：GRPO loss = ((policy_p / old_p) * advantage - beta * KL)，
        # 其中 (policy_p / old_p) 是相对概率比，advantage 是奖励信号，KL 是相对熵惩罚项
        # on-policy 方式中， old_p = policy_p （即每步 rollout 的所有回答都在本步更新），但分母不能求梯度，只有分子求梯度，这样才有梯度信号；
        # off-policy 方式中， old_p 来自之前的 checkpoint，也不能求梯度。
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GrandLine GRPO Training")
    
    # 训练参数
    parser.add_argument("--save_dir", type=str, default="../model_weight/grpo/exp_1")
    ...
    parser.add_argument("--swanlab_project", type=str, default="GrandLine-GRPO")
    ...
    def load_model(trainable=True):
        m = GrandLineForCausalLM(lm_config)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        g = torch.Generator()
        g.manual_seed(epoch)
        batch_sampler = SkipBatchSampler(train_sampler or torch.randperm(len(train_ds), generator=g).tolist(), 

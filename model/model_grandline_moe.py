...
from .config_moe import GrandLineMoEConfig
...
class Attention(nn.Module):
    def __init__(self, args: GrandLineMoEConfig):
        super().__init__()
...
class MoERouter(nn.Module):
    def __init__(self, config: GrandLineMoEConfig):
        super().__init__()
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        # 计算负载均衡 loss (DeepSeek style / switch transformer style)
        tokens_per_expert = torch.bincount(topk_indices.flatten(), minlength=self.num_routed_experts).float()
        expert_density = tokens_per_expert / (batch_size * seq_len * self.top_k)
        routing_density = routing_weights.mean(dim=0)
        aux_loss = torch.sum(expert_density * routing_density) * self.num_routed_experts
        
        return topk_weights, topk_indices, aux_loss


class GrandLineMoE(nn.Module):
    def __init__(self, config: GrandLineMoEConfig):
        super().__init__()
        ...
        self.shared_experts = nn.ModuleList([FeedForward(config) for _ in range(self.n_shared_experts)])
        self.routed_experts = nn.ModuleList([FeedForward(config) for _ in range(self.n_routed_experts)])

    def forward(self, hidden_states: torch.Tensor):
        batch_size, seq_len, hidden_dim = hidden_states.shape
        flat_hidden_states = hidden_states.view(-1, hidden_dim)
        
        # 1. Shared experts output
        shared_output = sum(expert(hidden_states) for expert in self.shared_experts)
        
        # 2. Routed experts output
        ...
        for i, expert in enumerate(self.routed_experts):
            ...
            if expert_mask.any():
                expert_inputs = flat_hidden_states[expert_mask.any(dim=-1)]
                expert_outputs = expert(expert_inputs)
                
                # Combine weights
                for k in range(topk_indices.shape[1]):
                    k_mask = (topk_indices[:, k] == i)
                    if k_mask.any():
                        routed_output[k_mask] += expert_outputs[k_mask[expert_mask.any(dim=-1)]] * topk_weights[k_mask, k].unsqueeze(-1)
...
class GrandLineBlock(nn.Module):
    """
    Transformer-Decoder 块：Masked-Self-Attention + feed_forward(MoE)
    采用 Pre-Norm 结构（Norm before attention/mlp）
    """
    def __init__(self, layer_id: int, args: GrandLineMoEConfig):
        super().__init__()
        ...
        self.feed_forward = GrandLineMoE(args)
    ...
    def forward(
        ...
        # FeedForward/MoE 部分
        ff_output, aux_loss = self.feed_forward(self.post_attn_rms_norm(x))
        x = x + ff_output
         
        return x, presnet_key_value, aux_loss
...
class GrandLineModel(nn.Module):
    ...
    def forward(self,
        ...
        for layer_idx, (layer, past_key_value) in enumerate(zip(self.layers, past_key_values)):
            hidden_states, present_key_value, aux_loss = layer(hidden_states,
                                                    position_embeddings,
        ...
        return hidden_states, present_key_values, total_aux_loss
...
class GrandLineForCausalLM(PreTrainedModel, GenerationMixin):
    ...
    """
    config_class = GrandLineMoEConfig
    
    def __init__(self, config: GrandLineMoEConfig = None):
        self.config = config or GrandLineMoEConfig()
    ...
    def forward(self, 
        ...
        if labels is not None:
            ...
            # 加入 MoE 的负载均衡 aux_loss
            if hasattr(self.config, 'moe_aux_loss_coeff'):
